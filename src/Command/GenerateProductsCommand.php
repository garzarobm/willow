...
class GenerateProductsCommand extends Command
{
        ...
        foreach ($statusCounts as $status => $count) {
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                if ($this->Products->save($product, ['associated' => ['Tags']])) {
                    $io->verbose(
                        __('Generated product: {0} (Status: {1})', $product->title, $product->verification_status),
                    );
                    $successCount++;
                } else {
    ...
    private function parseStatusDistribution(?string $distribution, int $totalQuantity, ConsoleIo $io): ?array
    {
        ...
        foreach ($pairs as $pair) {
            ...
            if (count($parts) !== 2) {
                $io->error(
                    __('Invalid status distribution format: {0}. Expected format: "status:count,status:count"', $pair),
                );
            ...
            if (!in_array($status, self::VALID_STATUSES, true)) {
                ...
                $io->error(
                    __('Invalid status: {0}. Valid statuses: {1}', $status, $validStatuses),
                );
