...
class ProductsController extends AppController
{
    ...
    // In ProductsController::view() 
    public function view($slug = null): ?Response
    {
        $product = $this->Products->findBySlug($slug)->firstOrFail();
        
        // Record product page view
        $this->loadModel('PageViews');
        $this->PageViews->recordProductView(
            $product->id,
    ...
    public function viewBySlug(string $slug): ?Response
    {
        ...
        if (empty($product)) {
            ...
            $slugEntity = $this->Slugs->find()
                ...
                ->select(['product_id', 'slug', 'foreign_key'])
                ->first();
